...
@section('breadcrumb')
    ...
    <span class="text-slate-700 font-medium">Edit Program</span>
@endsection
...
<div class="max-w-4xl space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Program Sosial</h1>
            <p class="text-slate-500 text-sm mt-0.5">Perbarui rincian kegiatan atau dokumen laporan sosial.</p>
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Edit Detail Program</h3>
        </div>
        <form method="POST" action="{{ route('socials.update', $social->id) }}" enctype="multipart/form-data">
            ...
            <div class="card-body space-y-6">
                ...
                <div>
                    ...
                    <input class="input @error('title') border-red-500 @enderror" type="text" id="title" name="title" value="{{ old('title', $social->title) }}" required />
                ...
                <div>
                    ...
                    <textarea class="input @error('description') border-red-500 @enderror" id="description" name="description" rows="4">{{ old('description', $social->description) }}</textarea>
                    @error('description') <p class="form-error">{{ $message }}</p> @enderror
                </div>

                {{-- Section Foto --}}
                <div class="p-4 rounded-xl border border-slate-200 bg-slate-50/50 space-y-3">
                    <label class="form-label font-semibold text-slate-800">Dokumentasi Foto</label>
                    @if ($social->image_path)
                        <div class="flex items-center gap-4">
                            <img src="{{ asset('storage/' . $social->image_path) }}" alt="" class="w-24 h-24 object-cover rounded-lg border border-slate-200 shadow-xs" />
                            <a href="{{ asset('storage/' . $social->image_path) }}" target="_blank" class="text-xs font-semibold text-sky-600 hover:underline">
                                Buka Foto &rarr;
                            </a>
                        </div>
                    @endif
                ...
                {{-- Section File --}}
                <div class="p-4 rounded-xl border border-slate-200 bg-slate-50/50 space-y-3">
                    <label class="form-label font-semibold text-slate-800">Berkas Dokumen Laporan</label>
                    @if ($social->file_path)
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ asset('storage/' . $social->file_path) }}" target="_blank" class="text-xs font-semibold text-emerald-600 hover:underline">
                                Unduh File &rarr;
                            </a>
                        </div>
                    @endif
